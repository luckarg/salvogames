...
function getGameData(gamePlayerId, viewShips){
	document.getElementById("dock").innerHTML = '<div id="display"><p>Ready?</p></div><div id="board"></div>'
	...
	.then(json => {
		 ...
		 data.gamePlayer.forEach(gamep =>{
//		             console.log(gamep);
		 	if(gamep.gamePlayerId == gp){
		 		player = gamep.player
		 	} else {
		 		opponent = gamep.player
...
//         console.log(opponent);
         if(data.gamePlayer.length==2){
             if(data.gamePlayer[0].gamePlayerId == gp){
                    playerInfo = [data.gamePlayer[0].player,data.gamePlayer[1].player];
//                    console.log(playerInfo);
                    document.getElementById("playerInfo").innerHTML = playerInfo[0].email+ ' (you)  vs  ' + playerInfo[1].email;
                    }
              else{
                   playerInfo = [data.gamePlayer[1].player,data.gamePlayer[0].player];
                   document.getElementById("playerInfo").innerHTML = playerInfo[1].email + '  vs  ' + playerInfo[0].email+' (you)';
                             }
                     }
                     else if(data.gamePlayer.length==1){
                       playerInfo = data.gamePlayer;
                       document.getElementById("playerInfo").innerHTML = 'Dear '+playerInfo[0].player.email + ', wait for another player to join you';
                      }
	})

	.then(function(){
             var salv = data.salvoes.sort(function(a, b){return b.turn - a.turn;});
             turns.myHits = salv.filter(x => x.player == player.id);
             turns.oppHits = salv.filter(x => x.player != player.id);
       })
...
function getSalvoes(salvoes, gp){
...
function shoot(shots,gp){
	let url = '/api/games/players/'+gp+'/salvoes'
	let init = {
		method: 'POST',
		headers: {
			"Content-Type": "application/json"
...
function aim(evt){
	if(!evt.target.classList.contains('hit')){
		if(document.querySelectorAll('.target').length < 5){
			...
			document.querySelector("#display p").innerText = 'to many shots'
		}
	} else{
		console.log('you already have shooted here')
		document.querySelector("#display p").innerText = 'you already have shooted here'
	}
